...
# Site recharge
construction <- sf::st_read("../data/lignes_construction/6301-23-0104_20230504_ZoneTransfertAdministrationF0110.shp") |> suppressMessages()
construction[,"nom"] <- "Recharge"

# Bathymetry
bathy0 <- sf::st_read("../data/bathymetrie.geojson") |> suppressMessages()
bathy10 <- sf::st_read("../data/bathymetrie10.geojson") |> suppressMessages()
bathy10[1,"depth"] <- "-10m"

# Sites
sites <- sf::st_read("../data/zones.geojson") |> suppressMessages()

# Sampling points
sampling_pts <- read.csv("../data/sample_pts_2023.csv")
sampling_pts$name <- tolower(sampling_pts$noms)
sampling_pts_supp <- read.csv("../data/sample_pts_supp_2023.csv") 
sampling_pts_supp$name <- tolower(sampling_pts_supp$noms)
...
# Import stations
stations <- terra::vect("../data/soussites.geojson") |> suppressMessages()
stations$noms_sous_sites <- tolower(stations$noms_sous_sites)

stations_not_started <- stations$noms_sous_sites
...
sampling_pts_todo <- sampling_pts
...
map <-# mapview(stations[stations$name %in% stations_finished,],
      ...
      #         layer.name = "Stations en cours") +
       mapview(sites,
               alpha.regions = 0.7) +
       mapview(stations[stations$noms_sous_sites %in% stations_not_started, "noms_sous_sites"],
